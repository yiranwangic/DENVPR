...
parameters {
  ...
  real log_B[Nt,4]; // transmission rate
  real phi;
  real b;
  real log_var;
...
transformed parameters {
  ...
  
  {
    ...
    for(t in 1:(Nt2-1)){
      ...
      for(s in 1:4) Mo[t+1,s] = Mo[t,s] + sigma*I1[t,s] - Mo[t,s]*(lamtot[t]-lam[t,s]) - gamma*Mo[t,s];
      for(s in 1:4) E2[t+1,s] = E2[t,s] + (sum(Mo[t,]) - Mo[t,s])*lam[t,s] - omega*E2[t,s] - gamma*E2[t,s];
    ...
    for(t in 1:Nt) for(s in 1:4) R0[t,s] = (S[t]+sum(Mo[t,])-Mo[t,s])*B[t,s]/sigma;
