...
set_cmdstan_path()
mod <- cmdstan_model('scenarios_AB.stan', pedantic=T)
fit <- mod$sample(data=data, chains=1, parallel_chains=1, iter_sampling=10, refresh=10, iter_warmup=10, save_warmup=TRUE, init=list(inits_chain1))
stanfit <- rstan::read_stan_csv(fit$output_files())

# Check convergence
chains <- rstan::extract(stanfit)
traceplot1 <- traceplot(stanfit, pars=c('init','lp__'))
traceplot2 <- traceplot(stanfit, pars=c('rho','b','log_var','log_phiNB'))
traceplot3 <- traceplot(stanfit, pars=c('init','lp__'),inc_warmup=T)
traceplot4 <- traceplot(stanfit, pars=c('rho','b','log_var','log_phiNB'),inc_warmup=T)
...
for(i in 1:data$Nt){
  for(s in 1:4){
    ...
    Rt[Rt$t==i & R0$sero==s,4:6] <- quantile(chains$Rt[,i,s], c(0.5,0.025,0.975))
