//----- 4-Serotype SEIR model  -----//
  // assumes immunity after 2 infections
// no age structure

data {
  
  int<lower=0> Nt; // N data time points
  int Nt2; // N time points for modelling
  int Ncases[Nt,4]; // Serotyped cases
  int NcasesTot[Nt]; // Non-serotyped cases
  real pSample[Nt]; // proportion of cases serotyped
  int pop[Nt]; // population
  real gamma; // birth/death rate
  real sigma; // 1/infectious period
  real omega; // 1/latent period
  real kappa; // human-mosquito-human transmission time
  int K;
  vector[K] alpha;
  int indL[Nt];
  int indU[Nt];
  int ind[Nt2];
  real phi;
  real delta;
  real zeta;
}


parameters {
  
  real <upper=0> log_rho; // reporting rate
  simplex[K] init;
  real log_B[Nt,4];
  real b;
  real log_var;
  real<lower=0> log_phiNB;
  
}


transformed parameters {
  
  real rho = exp(log_rho);
  real initS = init[1];
  real initR = init[2];
  vector[4] initMoP = init[3:6];
  vector[4] initMoS = init[7:10];
  vector[4] initE1 = init[11:14];
  vector[4] initI1 = init[15:18];
  vector[4] initE2 = init[19:22];
  vector[4] initI2 = init[23:26];
  
  vector[Nt] Ctot; // total reported cases
  real B[Nt,4] = exp(log_B); // transmission rate
  matrix[Nt,4] C; // total cases by serotype
  matrix[Nt,4] rC; // reported serotype cases
  matrix[Nt2-1,4] lam; // FOI
  matrix[Nt2,4] Mz;
  simplex[4] pSero[Nt];
  real R0[Nt,4];
  
  {
    vector[Nt2] S;
    matrix[Nt2,4] E1;
    matrix[Nt2,4] I1;
    matrix[Nt2,4] E2;
    matrix[Nt2,4] I2;
    matrix[Nt2,4] MoP;
    matrix[Nt2,4] MoS;
    vector[Nt2] R;
    real lamtot[Nt2];
    
    
    // initial conditions
    S[1] = init[1];
    R[1] = init[2];
    for(s in 1:4){
      MoP[1,s] = initMoP[s];
      MoS[1,s] = initMoS[s];
      E1[1,s] = initE1[s];
      E2[1,s] = initE2[s];
      I1[1,s] = initI1[s];
      I2[1,s] = initI2[s];
      Mz[1,s] = B[1,s]*(initI1[s]+phi*initI2[s]);
    }
    
    
    // loop through time & age
    for(t in 1:(Nt2-1)){
      
      for(s in 1:4) Mz[t+1,s] = Mz[t,s] + B[ind[t],s]*(I1[t,s] + phi*I2[t,s]) - kappa*Mz[t,s];
      for(s in 1:4) lam[t,s] = kappa*Mz[t,s];
      lamtot[t] = sum(lam[t,]);
      
      S[t+1] = S[t] - S[t]*sum(lam[t,]) - gamma*S[t] + gamma;
      for(s in 1:4) E1[t+1,s] = E1[t,s] + S[t]*lam[t,s] - omega*E1[t,s] - gamma*E1[t,s];
      for(s in 1:4) I1[t+1,s] = I1[t,s] + omega*E1[t,s] - sigma*I1[t,s] - gamma*I1[t,s];
      for(s in 1:4) MoP[t+1,s] = MoP[t,s] + sigma*I1[t,s] - zeta*MoP[t,s] - gamma*MoP[t,s];
      for(s in 1:4) MoS[t+1,s] = MoS[t,s] + zeta*MoP[t,s] - MoS[t,s]*(lamtot[t]-lam[t,s]) - gamma*MoS[t,s];
      for(s in 1:4) E2[t+1,s] = E2[t,s] + (sum(MoS[t,]) - MoS[t,s])*lam[t,s] - omega*E2[t,s] - gamma*E2[t,s];
      for(s in 1:4) I2[t+1,s] = I2[t,s] + omega*E2[t,s] - sigma*I2[t,s] - gamma*I2[t,s];
      R[t+1] = R[t] + sigma*sum(I2[t,]) - gamma*R[t];
    }
    
    for(s in 1:4) for(t in 1:Nt) C[t,s] = rho*omega*(sum(E2[indL[t]:indU[t],s]) + delta*sum(E1[indL[t]:indU[t],s]))*pop[t] +0.001;
    for(t in 1:Nt) Ctot[t] = sum(C[t,]); 
    for(t in 1:Nt) for(s in 1:4) rC[t,s] = C[t,s]*pSample[t] + 0.001;
    for(t in 1:Nt) for(s in 1:4) pSero[t,s] = sum(I1[indL[t]:indU[t],s]+I2[indL[t]:indU[t],s])/((sum(I1[indL[t]:indU[t],1:4])+sum(I2[indL[t]:indU[t],1:4])));
    for(t in 1:Nt) for(s in 1:4) R0[t,s] = (S[t]+sum(MoS[t,])-MoS[t,s])*B[t,s]/sigma;
  }
  
}



model {
  
  // Priors
  for(s in 1:4) log_B[1,s] ~ normal(-0.5,0.7);
  init ~ dirichlet(alpha);
  log_rho ~ normal(-2,0.5);
  log_phiNB ~ normal(0.5,0.5);
  b ~ normal(1,0.1);
  log_var ~ normal(-3,0.5);
  
  // likelihood
  for(s in 1:4) for(t in 2:Nt) log_B[t,s] ~ normal(b*log_B[t-1,s], exp(log_var));
  NcasesTot ~ neg_binomial_2(Ctot, exp(log_phiNB)); // neg-binomial total cases
  for(s in 1:4) Ncases[,s] ~ neg_binomial_2(rC[,s], exp(log_phiNB));
  
}


generated quantities {
  
  real predCases[Nt,4];
  real predCasesTot[Nt];
  
  for(s in 1:4) predCases[,s] = neg_binomial_2_rng(rC[,s], exp(log_phiNB));
  predCasesTot = neg_binomial_2_rng(Ctot, exp(log_phiNB));
  
}
